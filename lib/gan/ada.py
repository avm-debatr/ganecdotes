...
from lib.gan.optim import upfirdn2d
...
class AdaptiveAugment:
    def __init__(self,
        ...
        """
        -----------------------------------------------------------------------
        :param ada_aug_target: target augmentation probability for adaptive
                               augmentation
        :param ada_aug_len:    target duraing to reach augmentation probability
                               for adaptive augmentation
        :param update_every:   probability update interval of the adaptive
                               augmentation
        :param device:
        -----------------------------------------------------------------------
        """
    ...
    def tune(self, real_pred):
        ...
        -----------------------------------------------------------------------
        """
        # self.ada_aug_buf += torch.tensor((torch.sign(real_pred).sum().item(),
        #                                   real_pred.shape[0]),
        ...
        # check if the ada probability needs to be updated
...
def translate_mat(t_x, t_y, device="cpu"):
    """
    --------------------------------------------------------------------------
    Translation matrix for random affine transformation
...
def rotate_mat(theta, device="cpu"):
    """
    -------------------------------------------------------------------------
    Rotation matrix for random affine transformation
...
def random_apply_affine(img,
                        p,
                        G=None,
                        antialiasing_kernel=SYM6):
